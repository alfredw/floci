...
The downstream `persistent` backend commits unversioned bodies and metadata before
acknowledging writes and requires repair/restart after an I/O failure. See
[Storage Modes](../configuration/storage.md) for recovery and volume-retention
limits, and [Nerv storage evaluation](../nerv-storage-evaluation.md) for validation.
...
Set `FLOCI_SERVICES_S3_VALIDATE_HEADER_SIGNATURES=true` to verify S3 requests
using `Authorization: AWS4-HMAC-SHA256 ...`. The default remains permissive.
This setting checks the canonical request, signed headers, signing date (15-minute
clock-skew allowance), and `x-amz-content-sha256` against the received payload.
Invalid signatures return `403 SignatureDoesNotMatch` before object state changes.
Payload mismatches return `400 XAmzContentSHA256Mismatch`.

Use the existing local `test` / `test` credential pair, active IAM access keys,
or unexpired IAM session credentials with their matching signed session token.
Unknown keys are rejected. Both path-style and virtual-hosted request signatures
use the original wire URI.

The supported payload modes are a SHA-256 digest and `UNSIGNED-PAYLOAD`.
Streaming chunk-signature modes return `501 NotImplemented` in this opt-in mode.
Header signing uses `x-amz-date`; the legacy `Date`-header signing variant is not
implemented. Unsigned non-presigned S3 requests are rejected when enabled.

This setting does not enable IAM policy enforcement or change presigned URL
validation. Those retain their existing settings, including
`FLOCI_SERVICES_S3_ENFORCE_AUTH` for IAM-backed presigned SigV4 verification.
It also does not change storage persistence guarantees.
